...
# bc_leads <- read.csv("./data/bc_leads.csv")

mailer_cost <- read.csv("./data/mailer_cost.csv") %>%
  mutate(date_sent = lubridate::mdy(date_sent)) %>%
  rename("location" = city)

overall_cost <- read.csv("./data/overall_cost.csv")

last_update <- read.table("./out/lastupdate.txt")$V1
# TODO: don't join these
mailer_joined <- mailer_cost %>%
  full_join(ps_location, by = "zip_id")

mailer_locations <- mailer_joined %>%
  select(location, zip = zip_id) %>%
  distinct()
  

# functions --------

mailer_response <- function(mailer_joined, mailer_cost) {
    ...
    #   ungroup() %>%
    #   filter(as.Date(recruit_date) >= date_recode)
    
    # df_ps <- mailer_joined %>%
    #   filter(source == "direct_mail") %>%
    #   group_by(recruit_date, location, zip = zip_id) %>%
    #   count()
    # 
    
    # loc_onlyone <- ps_location %>%
    #   group_by(zip_id) %>%
    #   count() %>%
    #   filter(n > 1)
    ...
    df_ps <- ps_location %>%
      filter(
        source == "direct_mail",
        # zip_id %in% loc_onlyone$zip_id,
        recruit_date >= min(mailer_cost$date_sent)
        ) %>%
      group_by(recruit_date, location = city_id, zip_id) %>%
      count()
   
    ggplot(df_ps, aes(x = as.Date(recruit_date), y = n)) +
      geom_point() +

      # ylim(0, max(df_ps$n)) +
      facet_wrap( ~ zip_id) +
      geom_vline(data = mailer_cost, aes(xintercept = date_sent),
                 linetype = "dashed") +
      geom_text(data = mailer_cost, aes(
        x = date_sent,
        y = max(df_ps$n) - 2,
        label = date_sent
      ), check_overlap = TRUE,
      ...
      labs(x = "",
           y = "Prescreens",
           caption = "Each dot represents a day on which at least one prescreen was completed.")
   
}

mailer_selected <- function(mailer_joined, mailer_cost, selected) {
  df <- mailer_joined %>%
    filter(
  ...
  title <- paste("Mailer Response for", selected)
  
  ggplot(df, aes(x = as.Date(recruit_date), y = n)) +
    geom_point() +
    geom_smooth(se = FALSE) +
    geom_vline(data = mailer_cost, aes(xintercept = as.numeric(date_sent)),
               linetype = "dashed") +
    geom_label(data = mailer_cost, aes(
      x = date_sent + 5,
      y = max(df$n) - 1,
      label = date_sent
    )) +
    ylim(0, max(df$n)) +
    xlim(min(mailer_cost$date_sent), Sys.Date() + 90) +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    labs(x = "",
         y = "Prescreens",
         caption = "Each dot represents a day on which at least one prescreen was completed.",
...
rct_rate_table <- function(ps_location) {
    t <- ps_location %>%
        mutate(recruit_date = as.Date(recruit_date)) %>%
        group_by(
          recruit_date, source, screened, screen_status, 
          sl_status, ps_proj_eligible, recruit_int_summ, 
          randomized
          ) %>%
        count() %>%
        ungroup()
    ...
    ps_prior <- ps_location %>%
      filter(recruit_date <= Sys.Date() - 7) %>%
      group_by(source) %>%
      count(name = "lag_week")
    ...
    ps_result <- t %>%
      ...
      rename("uncategorized" = `NA`) %>%
        rowwise() %>%
        mutate(waiting_list = sum(waiting_list, uncategorized)) %>%
   ...
   tab <- tab %>%
     ...
     full_join(ps_prior, by = "source") %>%
     rowwise() %>%
     ungroup() %>%
     mutate(`change\npast week` = n_prescreens - lag_week)
    ...
    tab %>%
     select(
      ...
      mutate(pending = `Screening 1 Complete` + `Under LMP Review`) %>%
      select(-c(`Screening 1 Complete`, `Under LMP Review`)) %>%
     # mutate(across(
     #   .cols = -c(source, `total\nprescreens*`, `change\npast week`, lag_week),
    ...
    sidebarLayout(
        sidebarPanel(
            ...
            sliderInput(
                ...
                max = Sys.Date(),
                value = c(as.Date("2019-08-01"), Sys.Date())
            )
            ...
            tabsetPanel(
                ...
                tabPanel(
                    ...
                    plotOutput("mailer_response_plot",
                               height = "800px"),
                    selectInput("mailer_select",
                                label = "Select Location",
                                choices = mailer_locations$location),
                    br(),
                    br(),
                    plotOutput("mailer_select_plot")
                ),
                br(),
                br()
            ),
...
server <- function(input, output) {
    ...
    output$mailer_select_plot <- renderPlot({
      mailer_joined %>%
        filter(
          ps_proj_eligible %in% proj(),
          source %in% sources(),
        ...
        mailer_selected(mailer_cost, input$mailer_select)
    })
}
